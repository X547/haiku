...
static void scsi_insert_new_request( ScsiDeviceImpl *device,
	ScsiCcbImpl *new_request )
{
	ScsiCcbImpl *first, *last, *before, *next;
...
/*	{
		ScsiCcb *tmp = device->queued_reqs;
...
void scsi_remove_queued_request( ScsiCcbImpl *request )
{
	...
	if( static_cast<ScsiDeviceImpl*>(request->device)->queued_reqs == NULL )
		scsi_remove_device_queue( static_cast<ScsiDeviceImpl*>(request->device) );
}


// explictely unblock bus
static void scsi_unblock_bus_int( ScsiBusImpl *bus, bool by_SIM )
{
...
void ScsiBusImpl::Unblock()
{
	scsi_unblock_bus_int( this, true );
}


// explicitly unblock device
static void scsi_unblock_device_int( ScsiDeviceImpl *device, bool by_SIM )
{
...
void ScsiDeviceImpl::Unblock()
{
	return scsi_unblock_device_int( this, true );
}


// SIM signals that it can handle further requests for this bus
void ScsiBusImpl::ContSend()
{
...
static void scsi_block_bus_int( ScsiBusImpl *bus, bool by_SIM )
{
	SHOW_FLOW0( 3, "" );
...
void ScsiBusImpl::Block()
{
	return scsi_block_bus_int( this, true );
}


// explicitly block device
static void scsi_block_device_int( ScsiDeviceImpl *device, bool by_SIM )
{
	ScsiBusImpl *bus = device->bus;
...
void ScsiDeviceImpl::Block()
{
	return scsi_block_device_int( this, true );
}
