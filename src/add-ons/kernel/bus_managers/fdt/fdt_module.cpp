...
static status_t RegisterNode(fdt_bus* bus, int node, device_node* parentDev, device_node*& curDev)
{
	dprintf("RegisterNode('%s', %p)\n", fdt_get_name(gFDT, node, NULL), parentDev);
	...
	*attr++ = (device_attr){B_DEVICE_PRETTY_NAME, B_STRING_TYPE, {string: (strcmp(name, "") != 0) ? name : "Root"}};
	...
	*attr++ = (device_attr){"fdt/name", B_STRING_TYPE, {string: name}};
	...
	if (prop != NULL)
		*attr++ = (device_attr){"fdt/device_type", B_STRING_TYPE, {string: (const char*)prop}};

	prop = fdt_getprop(gFDT, node, "compatible", &propLen);
	if (prop != NULL)
		*attr++ = (device_attr){"fdt/compatible", B_STRING_TYPE, {string: (const char*)prop}};

	*attr = {0};

	status_t res = gDeviceManager->register_node(parentDev, "bus_managers/fdt/driver_v1", attrs, NULL, &curDev);
	if (res < B_OK)
		return res;
...
static void TraverseFdt(fdt_bus* bus, int &node, int &depth, device_node* parentDev)
{
	int curDepth = depth;
/*
...
{
	dprintf("fdt_bus_supports_device\n");
...
{
	dprintf("+fdt_bus_register_device\n");
	struct ScopeExit {
		ScopeExit() {dprintf("-fdt_bus_register_device\n");}
	} scopeExit;
	...
	return gDeviceManager->register_node(parent, "bus_managers/fdt/root/driver_v1", attrs,
		NULL, NULL);
}
...
{
	dprintf("fdt_bus_register_child_devices\n");
...
{
	dprintf("fdt_device_init_driver()\n");
...
{
	dprintf("fdt_device_uninit_driver()\n");
...
{
	dprintf("fdt_device_register_child_devices()\n");
	return B_OK;
}
...
fdt_device_get_name(fdt_device* dev)
{
	...
	uint32 fdtNode;
	ASSERT(gDeviceManager->get_attr_uint32(dev->node, "fdt/node", &fdtNode, false) >= B_OK);
...
fdt_device_get_reg(fdt_device* dev, uint32 ord, uint64* regs, uint64* len)
{
	...
	// TODO: use '#address-cells', '#size-cells' in parent node to identify field sizes
...
static bool
fdt_device_get_interrupt(fdt_device* dev, uint32 ord, device_node** interruptController, uint64* interrupt)
{
	ASSERT(dev != NULL);
	...
	int propLen;
	const void* prop = fdt_getprop(gFDT, (int)fdtNode, "interrupts-extended", &propLen);
	if (prop == NULL)
		return false;
