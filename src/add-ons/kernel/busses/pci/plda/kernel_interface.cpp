...
pci_controller_module_info gPciControllerDriver = {
	.info = {
		...
		.supports_device = PciControllerPlda::SupportsDevice,
		.register_device = PciControllerPlda::RegisterDevice,
		.init_driver = [](device_node* node, void** driverCookie) {
			return PciControllerPlda::InitDriver(node, *(PciControllerPlda**)driverCookie);
		},
		.uninit_driver = [](void* driverCookie) {
			return static_cast<PciControllerPlda*>(driverCookie)->UninitDriver();
		},
	},
	.read_pci_config = [](void* cookie,
		uint8 bus, uint8 device, uint8 function,
		uint16 offset, uint8 size, uint32* value) {
		return static_cast<PciControllerPlda*>(cookie)
	...
	.get_max_bus_devices = [](void* cookie, int32* count) {
		return static_cast<PciControllerPlda*>(cookie)->GetMaxBusDevices(*count);
	},
	.read_pci_irq = [](void* cookie,
		uint8 bus, uint8 device, uint8 function,
		uint8 pin, uint8 *irq) {
		return static_cast<PciControllerPlda*>(cookie)->ReadIrq(bus, device, function, pin, *irq);
	},
	.write_pci_irq = [](void* cookie,
		uint8 bus, uint8 device, uint8 function,
		uint8 pin, uint8 irq) {
		return static_cast<PciControllerPlda*>(cookie)->WriteIrq(bus, device, function, pin, irq);
	},
